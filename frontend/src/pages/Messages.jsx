    ...
    <div className="h-screen bg-[#0b0b0b] flex overflow-hidden">
      ...
      <main className="flex-1 p-8 lg:p-12 overflow-hidden flex flex-col">
        ...
        <div className="flex-1 flex gap-6 min-h-0">
          ...
          <div className="w-1/3 bg-[#111113] border border-white/5 rounded-[2.5rem] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-8 border-b border-white/5 bg-white/[0.02]">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-[#1a1a1c] border border-white/5 rounded-2xl py-3 pl-11 pr-4 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-[#ff5c00]/30 transition-all"
                />
              </div>
            </div>
                ...
                >
                  <div className="relative shrink-0">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-xl transition-all duration-300 ${
                      activeChat?._id === contact._id ? 'bg-white/20 text-white' : 'bg-[#1a1a1c] text-gray-400 group-hover:bg-[#2a2a2c]'
                    }`}>
                      {contact.name.charAt(0).toUpperCase()}
                    </div>
                    <span className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-4 border-[#111113] ${
                      activeChat?._id === contact._id ? 'bg-white' : 'bg-green-500'
                    }`}></span>
                  </div>
                  <div className="text-left flex-1 overflow-hidden">
                    <h3 className={`font-bold text-base truncate transition-colors duration-300 ${activeChat?._id === contact._id ? 'text-white' : 'text-white/90 group-hover:text-white'}`}>
                      {contact.name}
                    </h3>
          ...
          <div className="w-2/3 bg-[#111113] border border-white/5 rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl relative">
              ...
              {activeChat ? (
                ...
                >
                  ...
                  <div className="p-8 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-white tracking-tight leading-tight">{activeChat.name}</h2>
                    ...
                    <div className="flex items-center gap-3">
                      <button className="w-12 h-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all">
                    ...
                    {messages.length === 0 ? (
                      ...
                      <div className="space-y-6">
                        <div className="flex justify-center">
                          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-600 bg-white/5 px-4 py-1.5 rounded-full">Conversation Started</span>
                        </div>
                        {messages.map((msg, i) => {
                              ...
                              <div className={`max-w-[75%] group relative ${isMe ? 'items-end' : 'items-start'}`}>
                                <div className={`px-6 py-4 rounded-[1.8rem] shadow-lg transition-all duration-300 ${
                                  isMe 
                                    ...
                                    : 'bg-[#1a1a1c] text-white/90 border border-white/5 rounded-bl-none hover:bg-[#202022]'
                                }`}>
                  ...
                  <div className="p-8 border-t border-white/5 bg-white/[0.01]">
                    <form onSubmit={sendMessage} className="flex gap-4 items-center">
                      <button type="button" className="shrink-0 w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all">
                      ...
                      <div className="flex-1 relative">
                        <input
                          ...
                          className="w-full bg-[#1a1a1c] border border-white/5 rounded-2xl px-8 py-5 text-white placeholder:text-gray-600 focus:outline-none focus:border-[#ff5c00]/30 transition-all text-sm font-medium shadow-inner"
                        />
                      </div>
                      <button
                        type="submit"
                ...
                >
                  <div className="relative mb-10">
                    ...
                    <div className="absolute -top-2 -right-2 w-10 h-10 bg-[#ff5c00] rounded-2xl flex items-center justify-center shadow-lg border-4 border-[#111113]">
                      <span className="text-white text-lg">⚡</span>
                    </div>
                  </div>
                  <h2 className="text-3xl font-bold text-white mb-4 tracking-tight">Select a Chat</h2>
                  ...
                  <div className="mt-12 flex gap-4">
                    <div className="flex -space-x-3">
                      {[1,2,3,4].map(i => (
                        <div key={i} className="w-10 h-10 rounded-xl bg-[#1a1a1c] border-2 border-[#111113] flex items-center justify-center text-[10px] font-bold text-gray-500">
                          {String.fromCharCode(64 + i)}
                        </div>
                      ))}
                    </div>
                    <p className="text-xs text-gray-600 font-bold uppercase tracking-widest self-center">+ {contacts.length} Members</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
